...
from rigify.utils.bones import set_bone_widget_transform, align_bone_orientation
...
class Rig(SimpleChainRig):
    ...
    def initialize(self):
        ...
        if self.params.separate_rotation:
            self.separate_rotation = True
        else:
            self.separate_rotation = False
    ...
    def parent_mch_control_bones(self):
        if self.separate_rotation:
            self.set_bone_parent(self.bones.ctrl.master, self.bones.mch.rot)
    ...
    def configure_mch_follow_bones(self):
        if self.separate_rotation:
            ...
            self.make_property(master, 'root_follow', default=0.0)
            panel.custom_prop(master, 'root_follow', text='root_follow', slider=True)
    ...
    def rig_mch_follow_bones(self):
        if self.separate_rotation:
            ...
            self.make_driver(con, 'influence',
                            variables=[(self.bones.ctrl.master, 'root_follow')], polynomial=[1, -1])
    ##############################
    # UI
    ...
    def parameters_ui(cls, layout, params):
        ...
        layout.prop(params, 'separate_rotation')
